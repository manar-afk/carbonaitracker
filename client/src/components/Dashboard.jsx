...
export default function Dashboard({ user, logs, simulation, onDeleteLog, onNavigate }) {
  ...
  // Localized Performance Cache - Wrap calculations in useMemo (Resource Efficiency)
  ...
  // Calculate daily history for the past 7 days (Memoized)
  ...
  const maxEmissions = useMemo(() => {
    return Math.max(...dailyHistory.map(d => d.total), 5);
  }, [dailyHistory]);
  ...
  const handleDeleteClick = (id) => {
    if (window.confirm('Are you sure you want to delete this carbon activity log?')) {
      onDeleteLog(id);
    }
  };

  const statsBreakdown = [
    { name: 'Transit', value: calculatedStats.weekly.transport, color: 'hsl(var(--accent-cyan))', key: 'transport' },
    { name: 'Electricity', value: calculatedStats.weekly.electricity, color: 'hsl(var(--accent-emerald))', key: 'electricity' },
    { name: 'Food', value: calculatedStats.weekly.food, color: 'hsl(var(--accent-coral))', key: 'food' },
    { name: 'Shopping', value: calculatedStats.weekly.purchase, color: 'hsl(var(--accent-gold))', key: 'purchase' }
  ];
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
          <div className="streak-widget">
            <Flame size={24} style={{ color: 'hsl(var(--accent-gold))' }} aria-hidden="true" />
            <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
        ...
        <div className="glass-card">
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', margin: '12px 0 6px 0' }}>
            <span style={{ fontSize: '2.5rem', fontWeight: '800', color: 'hsl(var(--text-primary))' }}>{calculatedStats.daily.total}</span>
        ...
        <div className="glass-card">
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', margin: '12px 0 6px 0' }}>
            <span style={{ fontSize: '2.5rem', fontWeight: '800', color: 'hsl(var(--accent-emerald))' }}>{calculatedStats.weekly.total}</span>
            <span style={{ color: 'hsl(var(--text-muted))', fontWeight: '600' }}>kg CO₂e</span>
          </div>
          <p style={{ fontSize: '0.85rem', color: 'hsl(var(--text-secondary))', marginTop: '10px' }}>
      ...
      <div className="grid-dashboard">
        ...
        <div className="glass-card col-8">
          ...
          <div className="chart-container" style={{ height: '160px' }}>
            <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="100%" className="sparkline-svg" aria-label="Line chart showing carbon scores for the last 7 days">
              <defs>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {statsBreakdown.map(cat => {
                const percentage = calculatedStats.weekly.total > 0 ? (cat.value / calculatedStats.weekly.total) * 100 : 0;
                return (
                  <div key={cat.key}>
                    ...
                    <div style={{ width: '100%', height: '6px', background: 'hsla(var(--text-primary), 0.05)', borderRadius: '3px', overflow: 'hidden' }} role="progressbar" aria-valuenow={percentage} aria-valuemin="0" aria-valuemax="100" aria-label={`${cat.name} emissions percentage`}>
                ...
                <div key={log.id} className="activity-item animate-fade-in">
                  ...
                  <div className="activity-emissions">
                    ...
                    <button 
                      ...
                      aria-label="Delete carbon activity log entry"
                    >
